classdef XboxController < handle
    ...
    methods
        ...
        function [thrust,yaw,pitch,roll,calibrate,arm,override,land,setpointMode,setpointPrev,setpointNext] = getState(obj)
            ...
            thrust = uint8(min(max((obj.cntrlState.Gamepad.sThumbLY - obj.thumbLYOff) / 128.5, 0), 255));
            yaw = uint8(min(max((((obj.cntrlState.Gamepad.sThumbLX - obj.thumbLXOff) + 32768) / 257), 0), 255));
            pitch = uint8(min(max((((obj.cntrlState.Gamepad.sThumbRY - obj.thumbRYOff) + 32768) / 257), 0), 255));
            roll = uint8(min(max((((obj.cntrlState.Gamepad.sThumbRX - obj.thumbRXOff) + 32768) / 257), 0), 255));
