classdef PID_Controller 
    methods     ( Static = true )
        function [x_acc_d, pid_output, X_pid_err] = Xcontroller(X_pid, x_d, x, x_dot, dt)
            ...
            %% -------------------- FIRST PID BLOCK ------------------------------
            % Gains
            K_p = 50; % Shouldnt do anything with 100 (maybe 1 degree commanded) % 250,0,1 best for just commanding x
        ...
        function [y_acc_d, pid_output, Y_pid_err] = Ycontroller(Y_pid, y_d, y, y_dot, dt)
            ...
            %% -------------------- FIRST PID BLOCK ------------------------------
            % Gains
            K_p = 50; % Shouldnt do anything with 100 (maybe 1 degree commanded)
            K_i = 15;
            K_d = 25; %
